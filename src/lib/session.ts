...
export class TeslaBleSession {
  ...
  async getState(category: StateCategory, privateKey: CryptoKey): Promise<VehicleStateResult> {
  ...
  private async performHandshake(privateKey: CryptoKey): Promise<void> {
  ...
  private async sendEncryptedCommand(plaintext: Uint8Array): Promise<Uint8Array> {
    ...
    const encrypted = await encryptAesGcm(session.keys.aesKey, nonce, plaintext, aad);
  ...
  private async handleEncryptedResponse(message: any, raw: Uint8Array, requestId: Uint8Array): Promise<void> {
    ...
    const metadata = serializeMetadata(metadataItems);
    const aad = await sha256(metadata);
    const plaintext = await decryptAesGcm(session.keys.aesKey, nonce, combined, aad);
